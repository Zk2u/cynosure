//! A lightweight, reference-counted smart pointer for single-threaded async
//! executors.
//!
//! `LocalCell<T>` provides safe shared mutability without the overhead of
//! `RefCell`'s dynamic borrow checking. It's designed specifically for
//! single-threaded async environments like `monoio`.
//!
//! # Key Features
//!
//! - **Zero runtime overhead** - No borrow checking or panic-prone runtime
//!   checks
//! - **Compile-time safety** - Mutations only allowed in synchronous closures,
//!   preventing references held across await points
...
/// without having to use a `RefCell`.
